...
def test_insert_setting(mocker):
  ...
  mocker.patch('utilities.db.tbl_setting.get_setting_by_name', return_value=None)
  assert update_setting("setting_name", "setting_val") == True


def test_update_setting(mocker):
  mocker.patch('utilities.db.base.execuate', return_value=FakeCursor(1))
  mocker.patch('utilities.db.tbl_setting.get_setting_by_name', return_value=[0])
  assert update_setting("setting_name", "setting_val") == True
...
def test_get_all_setting(mocker):
  mocker.patch('utilities.db.base.execuate', return_value=FakeCursor(2, [[0,"name0","value0"], [1,"name1","value1"]]))
